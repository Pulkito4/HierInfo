...
def _extract_diverse_keywords(text: str, top_n: int = 8) -> List[str]:
    ...
    for keyword, score in candidates:
        ...
        is_redundant = any(
            clean_kw in ex.lower() or ex.lower() in clean_kw for ex in final_keywords
        )
        if not is_redundant:
...
def _generate_categories_from_keywords(
    keywords: List[str], max_categories: int = 4
) -> List[str]:
...
def apply_nlp_processing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df[["summary", "keywords", "categories"]] = pd.DataFrame(
        nlp_results.tolist(), index=df.index
    )
