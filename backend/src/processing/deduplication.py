...
def cluster_and_deduplicate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty or "embedding" not in df.columns or df["embedding"].isnull().all():
        logger.warning(
            "DataFrame is empty or missing embeddings. Skipping deduplication."
        )
        return df
    ...
    for cluster_id in unique_clusters:
        ...
        cluster_df = cluster_df.assign(
            source_rank=cluster_df["source_name"].apply(_rank_source)
        )
    ...
    deduplicated_df.drop(
        columns=["cluster_label", "source_rank"], inplace=True, errors="ignore"
    )

    logger.info(
        f"✅ Deduplication complete. Selected {len(deduplicated_df)} unique articles."
    )
