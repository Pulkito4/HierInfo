...
def parse_with_newspaper3k(url: str, timeout: int = 15) -> Optional[Dict]:
    ...
    try:
        ...
        # 1. Check for blocking phrases first.
        lower_content = content["raw_content"].lower()
        if any(phrase in lower_content for phrase in C.BLOCKING_PHRASES):
            logger.warning(
                f"❌ newspaper3k - Found blocking phrase in content from {content['domain']}"
            )
            return None

        # 2. Then, check for sufficient length.
        text_length = len(content["raw_content"])
        if text_length > 150:  # Increased threshold for better quality
            logger.info(
                f"✅ newspaper3k successful - {text_length} chars extracted from {content['domain']}"
            )
            return content
        else:
            logger.warning(
                f"❌ newspaper3k - insufficient content ({text_length} chars) from {content['domain']}"
            )
            return None
        ...
        with sync_playwright() as p:
            ...
            try:
                page.wait_for_selector(
                    "article, .article, .post-content, .entry-content", timeout=5000
                )
            except Exception:
                logger.debug(
                    "⚠️  No common article selectors found, proceeding with full page"
                )
            ...
            if any(phrase in lower_content for phrase in C.BLOCKING_PHRASES):
                logger.warning(
                    f"❌ Playwright - Found blocking phrase in content from {content['domain']}"
                )
                return None
            ...
            if text_length > 150:
                logger.info(
                    f"✅ Playwright successful - {text_length} chars extracted from {content['domain']}"
                )
                return content
            else:
                logger.warning(
                    f"❌ Playwright - insufficient content ({text_length} chars) from {content['domain']}"
                )
                return None
...
def parse_article_with_two_tier_fallback(url: str) -> Optional[Dict]:
    ...
    for tier, (method_name, parser_func, description) in enumerate(parsing_methods, 1):
        ...
        try:
            ...
            if (
                content
                and content.get("raw_content")
                and len(content["raw_content"]) > 100
            ):
                logger.info(
                    f"🎉 SUCCESS! {method_name} extracted {len(content['raw_content'])} characters from {domain}"
                )
                ...
                logger.warning(
                    f"⚠️  {method_name} returned insufficient content for {domain}, trying next method..."
                )
...
def parse_articles_batch(urls: List[str], max_workers: int = 5) -> List[Dict]:
    ...
    logger.info(
        f"🚀 Starting batch parsing of {len(urls)} articles with {max_workers} workers"
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_url):
            ...
            try:
                ...
                failed_count += 1
                logger.error(
                    f"💥 Batch parse exception for {urlparse(url).netloc}: {e}"
                )
    ...
    logger.info(
        f"📊 Batch parsing completed: {success_count}/{total_count} articles successful ({success_rate:.1f}%)"
    )
